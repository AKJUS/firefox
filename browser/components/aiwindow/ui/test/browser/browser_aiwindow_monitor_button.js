...
add_task(async function test_monitor_panel_create_seeds_current_page() {
  ...
  try {
    ...
    // The host seeds the current page into the form asynchronously,
    // so wait for the pill to render
    await TestUtils.waitForCondition(
      () => form.shadowRoot.querySelectorAll(".page-pill").length === 1,
      "Waiting for the seeded page to render as a pill"
    );
    Assert.deepEqual(
      form.pageUrls,
      ["https://example.com/"],
